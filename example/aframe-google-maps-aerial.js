...
AFRAME.registerComponent('google-maps-aerial', {
  ...
  init: function() {
    ...
    // Register plugins
    this.tiles.registerPlugin(new GoogleCloudAuthPlugin({ apiToken: this.data.apiToken }));
  ...
  update: function(oldData) {
    ...
    if (this.tiles && (oldData.latitude !== this.data.latitude || oldData.longitude !== this.data.longitude)) {
      this.tiles.setLatLonToYUp(
        this.data.latitude * MathUtils.DEG2RAD,
        this.data.longitude * MathUtils.DEG2RAD
      );
